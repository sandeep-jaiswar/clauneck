...
class StatisticalAnalysisSolver(SolverBase):
    ...
    def _resolve_argument(self, arg: str, quantities: Dict[str, Quantity]) -> Union[float, List[float], None]:
        ...
        For multi-dimensional arrays, flattens to 1D to ensure consistent treatment.
        """
        ...
        if arg in quantities:
            ...
            if qty.value is not None:
                value = qty.value
                # If value is a nested list/array (e.g., 2D matrix), flatten it
                # This ensures consistent behavior for matrix inputs
    ...
    def _stat_mode(self, args: List) -> dict:
        ...
        data = np.array(args[0])
        if len(data) == 0:
            return {"success": False, "error": "mode() requires a non-empty dataset"}
        mode_result = stats.mode(data, keepdims=True)
        mode_value = float(mode_result.mode[0])
        # Validate mode is not NaN (edge case for datasets with no repeated values)
        if np.isnan(mode_value):
            return {"success": False, "error": "mode() could not compute (no repeated values in dataset)"}
        return {"success": True, "mode": mode_value}
